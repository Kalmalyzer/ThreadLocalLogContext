...
/*
 * ThreadLocal.cs from .NET 4.7.1 RS3 backported to Unity's .NET 3.5
 *
 * Changes required:
 * - Commented out reference to System.Diagnostics.Contracts
 * - Replaced all Environment.GetResourceString("message") -> Environment_GetResourceString_Replacement("message") which implements as a no-op
 * - Commented out Debugger.NotifyOfCrossThreadDependency()
 * - Added UnityEngine.Assertions
 * - Replaced Contract.Assert() with Assert.IsTrue()
 * - Disabled size tests against Array.MaxArrayLength
 */
...
#region IDisposable Members
        ...
        protected virtual void Dispose(bool disposing)
        {
            ...
            lock (s_idManager)
            {
                ...
                {
                    Assert.IsTrue(id >= 0 || !m_initialized, "expected id >= 0 if initialized");
        ...
        private static string Environment_GetResourceString_Replacement(string s)
        {
            return s;
        }
        ...
        private T GetValueSlow()
        {
            ...
            if (id < 0)
            {
                throw new ObjectDisposedException(Environment_GetResourceString_Replacement("ThreadLocal_Disposed"));
            }
            ...
            else
            {
                ...
                if (IsValueCreated)
                {
                    throw new InvalidOperationException(Environment_GetResourceString_Replacement("ThreadLocal_Value_RecursiveCallsToValue"));
                }
            }
        ...
        public IList<T> Values
        {
            get
            {
                if (!m_trackAllValues)
                {
                    throw new InvalidOperationException(Environment_GetResourceString_Replacement("ThreadLocal_ValuesNotAvailable"));
                }

                var list = GetValuesAsList(); // returns null if disposed
                if (list == null) throw new ObjectDisposedException(Environment_GetResourceString_Replacement("ThreadLocal_Disposed"));
                return list;
            }
        }
        ...
        {
            Assert.IsTrue(table.Length < minLength);
                ...
                for (int i = 0; i < slotArray.Length; i++)
                {
                    ...
                    else
                    {
                        ...
                        lock (s_idManager)
                        {
                            ...
                            Assert.IsTrue(linkedSlot.Previous != null);
